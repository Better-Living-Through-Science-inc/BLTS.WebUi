...
{
    public abstract class WebUiWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static WebUiWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(WebUiWebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<WebUiDbContext> action)
        {
            using (var context = IocManager.Resolve<WebUiDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<WebUiDbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<WebUiDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<WebUiDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<WebUiDbContext, Task<T>> func)
        {
            T result;
